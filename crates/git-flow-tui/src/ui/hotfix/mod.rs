...
use crate::ui::{AppState, UiIface};

use git_flow_tui_core::{git::GitWrapper, others::whiteboard::WHITEBOARD};
use ratatui::{
...
impl UiIface for HotfixList {
    fn render(&mut self, _header: Rect, body: Rect, _footer: Rect, frame: &mut Frame) {
        self.list = Some(GitWrapper::global().lock().unwrap().get_hotfixes().unwrap());
    ...
    fn handle_input(&mut self, key: KeyCode) -> Option<AppState> {
        match key {
            ...
            KeyCode::Char('+') => return Some(AppState::HotfixStart),
            ...
            KeyCode::Delete => {
                    ...
                    && let Some(branch) = list.get(selected)
                {
                    ...
                    return Some(AppState::HotfixFinish);
                }
            }
            _ => (),
        }
