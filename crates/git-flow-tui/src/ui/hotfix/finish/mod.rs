...
use git_flow_tui_core::{logic::hotfix::hotfix_finish, others::whiteboard::WHITEBOARD};
use ratatui::{
...
impl UiIface for HotfixFinish {
    fn render(&mut self, header: Rect, body: Rect, footer: Rect, frame: &mut Frame) {
        self.set_text("Finish hotfix".to_string(), header, frame);
        ...
        match self.state {
            FinishProcState::Confirm => {
                let widget = MultiChoice::new(
                    &format!("Are you sure you want to finish '{}' hotfix?", self.name),
            ...
            KeyCode::Enter if self.state == FinishProcState::Confirm => {
                ...
                if selected == 0 {
                    ...
                    self.worker = Some(thread::spawn(move || {
                        if let Err(e) = hotfix_finish(&name, tx) {
